...
\label{sec:orgfac6510}

\subsubsection{Needed Definitions}
\label{sec:orgbedd06c}
To begin, we need to modify the \hyperref[sec:orgd9758a9]{Newton's Universal Gravitation Law} to fit the parameters of the scenario. Namely, we need to treat both Earth and our object as point masses, and assign \(M_1\) to be Earth and \(M_2\) to be our object.
...
\label{sec:org0a4d9c4}
To deduct the escape velocity of the Earth, our object \(M_2\) must do enough work such that the work Earth exerts upon it --- \hyperref[sec:org43d4e10]{as deducted above} --- is canceled out.

\subsubsection{Calculation Setup}
\label{sec:orgc092b96}
...
Because of the fact that \(\vec{r_{CM}} = \frac{1}{M} \sum^n_{i=1} m_i \vec{r_i}\) as derived \hyperref[sec:org70c98ac]{above}, \(\vec{r_{CM}}\) changes when the system as a whole moves. However, internal forces does not do this, meaning the existence of internal forces does not change \(\vec{r_{CM}}\).
...
Applying the expression for the center of mass \hyperref[sec:org70c98ac]{above}, we deduct that the center of mass of this object is located at point \((-0.7, 2.3, 2.8)\).
