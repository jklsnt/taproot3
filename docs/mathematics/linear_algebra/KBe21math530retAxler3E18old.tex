...
\sout{If \(V\) is finite dimensional, suppose \(v_1, \ldots, v_n\) is a basis of \(V\) and \(v_1, \ldots, v_k\) is a basis of \(U\) (\(k = \text{dim } U\) and \(n = \text{dim }V\)).}
\sout{For each \(k < j \le n\), \(\pi v_j \neq 0\), and we can control where \(S\) should send it. Let \(S\) be defined by:}
\[ S( \pi v_j) = T v_j \]
\sout{Then, \(S \circ \pi\) will send each vector in \(U\) to 0 and each other vector where \(T\) would send it. Because \(U \subseteq \text{null }T\), \(S \circ \pi = T\).}

\sout{This argument does not work for infinite dimensional vector spaces. Instead, perhaps we can send anything not in \(U\) to where \(T\) would send it and show that the resulting \(S\) is linear? I'm not convinced by the following argument:}

\sout{Let \(S : V/U \to W\) s.t. \(S(\pi v) = Tv\). Then, \(S \circ \pi = T\).}

\sout{For \(S\) to be linear, it needs to be additive and homogenous. For \(u, v \in V\) and \(\lambda \in \mathbb F\),}
    \sout{. \[ S\pi u + S\pi v = Tu + Tv = T(u+v) = S(\pi u + \pi v) \] .}
    \sout{. \[ \lambda S \pi u = \lambda T u = T(\lambda u) = S (\lambda \pi u) \] .}

\sout{In other words, \(T\) is linear thus \(S \circ \pi\) is also linear.}
...
\subsection{define S(U + v) = T v}
\label{sec:org9f74fb6}

\subsubsection{check that it is well defined}
\label{sec:orgff501d8}

\begin{enumerate}
\item every element is sent to exactly one place
\label{sec:orgb23e2a9}
\end{enumerate}


\subsubsection{check that linearity is inhereted from T}
\label{sec:orgf5c98d5}

\section{Reverse Direction by Contrapositive}
\label{sec:org9c03e7d}
