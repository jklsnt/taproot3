...
They are composed of an encoder and decoder, each of which itself is actually a stack of six smaller encoders/decoders. These encoder/decoder layers (which I'll refer to as just encoder layers for now) are composed of an [[file:20200925082542-attention.org][attention]] layer and a feedforward neural network. 
