...
First, every person's note $N_{i}$ is turned into a product category $N_{i} \times \{i\}$ in the category $\mathsf{Olg} \times \mathsf{Set}$. This category should be isomorphic to the original note category. Unless otherwise specified, we will be dealing with objects in $\mathsf{Olg} \times \mathsf{Set}$
...
After an aggregate category is created, a more condensed category $N_{cond}$ is created. This is done by a functor $F: N\rightarrow N_{cond}$ that takes every object in $N$ with the same idea to the same object in $N_{cond}$. 
