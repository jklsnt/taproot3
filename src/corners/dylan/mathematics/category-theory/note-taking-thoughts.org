...
After an aggregate category is created, a more condensed category is created. This is done by a functor $F: N \rightarrow N_{cond}$ where for any two objects $(X, i), (Y, j) \in N$, if $X = Y$, then 
