...
First, every person's note $N_{i}$ is turned into a product category $N_{i} \times \{i\}$. This category should be isomorphic to the original note category.
...
After an aggregate category is created, a more condensed category $N_{cond}$ is created. If we think of a functor $D: \mathsf{Olg} \times Set$
