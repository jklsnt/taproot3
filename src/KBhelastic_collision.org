...
Lastly, we understand that the angular momentum is also conserved; setting the origin at the centre of mass:

\begin{equation}
   v_0 (m_1r^2) = v_f (m_1 r^2) + v_{CM} I_0
\end{equation}
...
solve([m1*v0 == m1*vf+m2*vcm, m1*v0^2==m1*vf^2+m2*vcm^2+I*w^2, v0*m1*r^2 == vf*m1*r^2+vcm*I], vf, vcm, w)
#+end_src
