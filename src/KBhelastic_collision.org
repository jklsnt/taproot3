...
# expand(solve([m1*v0 == m1*vf+m2*vcm, 2*m1*v0^2==2*m1*vf^2+m2*vcm^2+I*w^2, m1*r^2*(v0/r)==m1*r^2*(vf/r)+I*w], vf, vcm, w, to_poly_solve=True))
# #+end_src

# #+RESULTS:
# : (I, m1, v0, m2, r, vf, vcm, w)
# : [[vf == (m1*m2*r^2 + I*m1 - 2*I*m2)*v0/(m1*m2*r^2 + I*m1 + 2*I*m2), vcm == 4*I*m1*v0/(m1*m2*r^2 + I*m1 + 2*I*m2), w == 4*m1*m2*r*v0/(m1*m2*r^2 + I*m1 + 2*I*m2)], [vf == v0, vcm == 0, w == 0]]
