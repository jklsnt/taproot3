...
\end{equation}
 ...
 #+begin_src sage
var("rho phi theta")
M = matrix([[cos(phi)*cos(theta), -rho*cos(phi)*sin(theta), -rho*sin(phi)*cos(theta)], [cos(phi)*sin(theta), rho*sin(phi)*cos(theta), rho*cos(phi)*sin(theta)], [cos(phi), 0, -rho*sin(phi)]])
M.det().full_simplify()
 #+end_src
