...
*** Optimizing for ${x_f}^2$ via $h_0$ via $\frac{d{x_f}^2}{h_0}$
We know that, by optimizing for ${x_f}^2$, $x_f$ is optimized due to the setup of the problem of the behavior of the line as a specific length. 
