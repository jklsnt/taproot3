...
We quickly return to the following expression:

\begin{equation}
    \theta_0 =& arctan(\frac{2(H-h_0)}{x_f})
\end{equation}

We write an expression of $x_f$, that:

\begin{align}
&tan(\theta_0) = \frac{2(H-h_0)}{x_f} \\
\Rightarrow\ &x_f = \frac{2(H-h_0)}{tan(\theta_0)}
\end{align}

We then optimize $x_f$ w.r.t. $\theta_0$.

\begin{align}
    \frac{d}{d \theta_0} x_f &= 2(H-h_0) \frac{d}{d \theta_0} cot(\theta_0) \\
    &= 2(H-h_0) -csc^2(\theta_0)
\end{align}
